...
contract('CoinHodlCoin', async accounts => {
    ...
    it('it is able to add in white list', async function(){
        await CoinHodlCoin.addToWhiteList(accounts[0]);
        let member = await CoinHodlCoin.whiteList(accounts[0]);
    ...
    it('it is able to send 1 CH24 form 0 to 1', async function(){
        let balance = await CoinHodlCoin.balanceOf(accounts[0]);
        assert.equal(balance.valueOf(), ether(10000000 - 1));
        ...
        let timeLimit = await CoinHodlCoin.timeLimit(accounts[1]);
        assert.equal(parseInt(timeLimit.valueOf()), 0);
    });

    it('it is not able to send 1 CH24 before adding to white list', async function(){
        await CoinHodlCoin
            .transfer(accounts[2], ether(1), { from: accounts[1] })
            .should
            .be
    ...
    it('it is able to add in white list', async function(){
        await CoinHodlCoin.addToWhiteList(accounts[1]);
        let member = await CoinHodlCoin.whiteList(accounts[1]);
        assert.equal(member.valueOf(), true);
    });

    it('it is able to send 1 CH24 after adding to white list', async function(){
        let balance = await CoinHodlCoin.balanceOf(accounts[1]);
        assert.equal(balance.valueOf(), ether(1));
        ...
        balance = await CoinHodlCoin.balanceOf(accounts[2]);
        assert.equal(balance.valueOf(), ether(2));
    });

    it('it is able to remove from white list', async function(){
        await CoinHodlCoin.removeFromWhiteList(accounts[1]);
        let member = await CoinHodlCoin.whiteList(accounts[1]);
